...
char input_uniqueid[20];        //used when admin/FSO logs in 
char input_stallid[20];         //used by customers to find stall details via stall id
char input_foodtype[20];        //used by customers to find stall details via food type 

/*********************************************************************************************************************************/

//////////////////////////////////////////////// Class Admin /////////////////////////////////////////////////////////////////////

/* Contains all the functions necessary. Interacts with class MainMenu to show the right functions at the right time. Has 2 objects. */

class Admin{
    private:
        //following are the variables that store various details of food stalls
        int total_items;
        float price[10];     
        char name[20];
        char food_menu[10][10], food_type[20];

        //following are the variables that store login password and recovery password for login
        char password[20], recovery_password[20];

    protected:
        //more details about the food stalls
    ...
    public:
        ...
        //member functions
        int login();                            //for validation of login info
        int recover();                          //retrieves password by asking for the recovery password
        void set_rest_data();                   //the admin can enter details of a new restaurant
        void rest_display();                    //displays the restaurant details
        int rest_profile();                     //the admin/SFO can access the overview of the restaurant   (on putting unique id)
        int know_rest_menu();                   //the admin/SFO can see the restaurant menu     (on putting unique id)
        void add_rest_menu();                   //the admin/SFO can add menu items (on putting unique id)
        void del_rest_menu();                   //the admin/SFO can delete some item from menu
        void mod_rest_profile();                //the admin/SFO can modify email/mob. no./rest-type of a stall
        int display_rest_menu_to_customer();    //the customer can see the menu bu entering the stall id
        void redisplay_menu_to_customer();      //to display the menu to the cutomer again
        int search_rest_by_foodtype();          //the customer can search for the stall menus by entering food type
        void invalid_input();                   //a message shows when some invalid input is entered
        
}admin, owner; 
//only some of the functions are avaialable for customers, so, they don't need another object. Instead we can just call the needed function with the help of admin object   

/*******************************************************************************************************************************/

////////////////////////////////////////////  class Admin member functions  ////////////////////////////////////////////////////
...
/* Contains functions to facilitate calls of the Admin class member functions. Contains only 1 object. */
...
/* Shows the Main Menu when program runs. Calls the appropriate function of class MainMenu */
